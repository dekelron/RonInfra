...
Seed sweep for the per-frequency lambda dip at prob: seed 1 of 3. vgg19_bn carries the largest dip (+0.54) and has no valid scrambled control, so seed spread is the only null available for it.
...
|---|---|---|---|---|---|---|---|
| +0.51 | +0.20 | -0.19 | -0.52 | -1.31 | -0.70 | -0.18 | -0.35 |

Bands: low (1, 1.75) **+0.36**, mid (7–28) **-0.84**,
high (56, 75) **-0.26** — so this run dips in the mid band,
**0.58** below both ends. λ at `prob` = -0.268 at λ-R² 0.950,
mean R² 0.837.
...
```
run.py --model vgg19_bn --reps 50 --seed 1 --save-run results/vgg19-bn-r50-s1 --notes Seed sweep for the per-frequency lambda dip at prob: seed 1 of 3. vgg19_bn carries the largest dip (+0.54) and has no valid scrambled control, so seed spread is the only null available for it. --figures out/ --layers all
```
